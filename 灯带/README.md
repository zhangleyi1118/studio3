...
| 组编号 | 包含灯带 | ESP32引脚 | 说明 |
|-------|---------|----------|------|
| 组1   | 灯带1-4 | GPIO 4   | 数据线（Data）- 同组灯带数据线并联 |
| 组2   | 灯带5   | GPIO 16  | 数据线（Data） |
| 组3   | 灯带6   | GPIO 17  | 数据线（Data） |
| 组4   | 灯带7-10| GPIO 18  | 数据线（Data）- 同组灯带数据线并联 |
| 组5   | 灯带11-14| GPIO 19  | 数据线（Data）- 同组灯带数据线并联 |

**注意**：同组内的灯带数据线（DI）需要并联连接到对应的GPIO引脚。
...
**重要**：同组内的灯带数据线需要**并联**连接到同一个GPIO引脚。

1. **组1（灯带1-4）**：将4根灯带的DI引脚并联后连接到 **GPIO 4**
2. **组2（灯带5）**：将灯带5的DI引脚连接到 **GPIO 16**
3. **组3（灯带6）**：将灯带6的DI引脚连接到 **GPIO 17**
4. **组4（灯带7-10）**：将4根灯带的DI引脚并联后连接到 **GPIO 18**
5. **组5（灯带11-14）**：将4根灯带的DI引脚并联后连接到 **GPIO 19**

**并联方法**：
- 使用面包板或接线端子将同组灯带的数据线连接在一起
- 然后从并联点连接到对应的ESP32 GPIO引脚
- 确保连接牢固，避免接触不良
